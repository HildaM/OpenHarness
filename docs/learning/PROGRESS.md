...
**已完全看透主线 + 四大子系统**：`oh` 命令 → CLI 解析 → 模式路由 → 运行时装配 → Agent 循环 → LLM 调用 → 工具执行 → 流式返回。

工具系统、API 客户端、权限系统和多 Agent 协作（Swarm Leader-Worker 架构 + 4 种后端 + ContextVar 隔离 + 文件邮箱 + Git Worktree）均已深入理解。
...
### C. 权限系统 ✅ 已完成（→ 13-permission-system.md）
...
*下次学习时，从方向 D（命令系统）开始，1374 行的 registry.py，按需查阅。*
